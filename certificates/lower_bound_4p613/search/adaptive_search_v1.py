...
def main():
 ...
 if args.resume:
  r=np.load(args.resume+'.npz');meta=json.loads(Path(args.resume+'.json').read_text());sites=[tuple(map(F,p)) for p in meta['sites']];w=r['w'].copy();poses=r['poses'].copy();A=r['A'].copy();step=int(r['steps'])
 else:
  if args.seed:
 ...
 for it in range(args.iterations):
  ...
  obj=problem.sz*(1+1e-8*np.random.default_rng(17).uniform(size=problem.nv))
  dual=None
  ...
  if not args.no_columns and it>=2 and problem.sz@w>16.995 and columns_rounds<16:
   tm=time.time();priced=problem.price(dualposes,dual,it);existing=set(problem.sites);newsites=[]
   for score,x,y in priced:
    ...
    newsites.append(pair);existing.add(pair)
